    ...
    if(jobItem) {
      ...
      if(jobItem.ingredients.length > 0) {
        jobItem.ingredients.forEach(function(ing) {
          var ingItem = getIngredientItem(ing.id);
          if(ingItem) {
            ingItem.totalCost = parseFloat(ingItem.costPerPortion) * parseFloat(ing.quantity);
            jobItem.totalIngCost += parseFloat(ingItem.totalCost);
          }
        });
        jobItem.prepCostPerPortion = Math.round((jobItem.totalIngCost/jobItem.portions) * 100)/100;
        jobItem.totalIngCost = Math.round(jobItem.totalIngCost);
      }
      return jobItem;
    }
  }
...
getIngredientItem = function(id) {
  if(id) {
    ...
    if(item) {
      ...
      if(item.unit == "each") {
        item.costPerPortion = parseFloat(item.costPerUnit)/parseFloat(item.unitSize)
      } else {
        var unitId = item.unit + "-" + item.portionUsed;
        if(unitId) {
          var conversion = Conversions.findOne(unitId);
          if(conversion) {
            var convertedCount = parseFloat(conversion.count);
            if(item.unitSize > 1) {
              convertedCount = (convertedCount * parseFloat(item.unitSize));
            }
