...
UI.registerHelper('calculatedIngCost', function(id) {
  var cost = 0;
  var ing = Ingredients.findOne(id);
  var doc = Session.get("thisPrepItemQty");
  if(doc.id == id) {
    cost = (parseInt(ing.costPerUnit)/parseInt(ing.unitSize)) * parseInt(doc.qty);
    return cost;
  }
});
