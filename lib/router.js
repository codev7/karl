...
Router.route('/menuItem/:_id', {
  ...
  waitOn: function() {
    var cursors = [];
    cursors.push(subs.subscribe("menuItem", this.params._id));
...
Router.route('/reports/:week', {
  ...
  waitOn: function() {
    var cursors = [];
    cursors.push(subs.subscribe("usersList"));
    var week = getDatesFromWeekNumber(this.params.week);
    var shifts = weeklySubs(week[0].date);
    cursors.push(shifts);
    ...
    if(userIds.length == users.length) {
      var payroll = subs.subscribe("weeklyPayroll", week[0].date, week[6].date, userIds);
      cursors.push(payroll)
    }
    return cursors;
  },
  data: function() {
