...
//--------------------MENU ITEMS ROUTES
Router.route('/menuItems/:category', {
  ...
  waitOn: function() {
    ...
    if(this.params.category) {
      cursors.push(Meteor.subscribe("menuList", this.params.category));
    }
    return cursors;
  },
  data: function() {
...
function homePageCursors() {
  var cursors = [];
  var menuItemCursor = subs.subscribe("menuList");
  cursors.push(menuItemCursor);
  var jobItemCursor = subs.subscribe("allJobItems");
  cursors.push(jobItemCursor);
  var ingredientsCursor = subs.subscribe("allIngredients");
  cursors.push(ingredientsCursor);
  return cursors;
}
