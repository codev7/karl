...
Router.route('/menuItem/:_id', {
  ...
  waitOn: function() {
    ...
    cursors.push(subs.subscribe("allJobItems"));
    // var menu = MenuItems.findOne(this.params._id);
    // var ings = [];
    // var preps = [];
    ...
    //     ings.push(ing._id);
    //   });
    //   cursors.push(subs.subscribe("ingredients", ings));
    // }
    // if(menu && menu.jobItems.length > 0) {
    //   menu.jobItems.forEach(function(job) {
    //     preps.push(job._id);
    //   });
    //   cursors.push(subs.subscribe("jobItems", preps));
    // }
    // cursors.push(subs.subscribe("allCategories"));
    // cursors.push(subs.subscribe("allStatuses"));
    cursors.push(subs.subscribe("allIngredients"))
...
Router.route('/roster/weekly/:week', {
  ...
  waitOn: function() {
    if(this.params.week != null) {
      var cursors = [];
      cursors.push(weeklySubs(this.params.week));
...
Router.route('/roster/template/weekly', {
  ...
  waitOn: function() {
    var cursors = [];
    cursors.push(subs.subscribe("shiftTemplates")); 
    cursors.push(subs.subscribe("workers"));
    cursors.push(subs.subscribe("allSections"));
    return cursors;
  },
...
function dailySubs(date, user) {
  ...
  var shifts = Shifts.find({"shiftDate": new Date(date).getTime()}).fetch();
  if(shifts.length > 0) {
    var jobs = [];
    shifts.forEach(function(shift) {
      subscriptions.push(subs.subscribe("jobs", shift.jobs));
    });
  }
...
function menuItemSubs(id) {
  var cursors = [];
  var menuItemCursor = Meteor.subscribe("menuItems", [id]);
